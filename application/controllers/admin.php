...
class Admin extends CI_Controller{
  ...
  function datarute(){
    ...
    $this->load->view('v_datarute',$data);
...
//maskapai//
  ...
  function datamaskapai(){
    $data['transportation'] = $this->data_crud->tampil_datamaskapai()->result();
    $this->load->view('v_datamaskapai',$data);
}
    function hapus_datamaskapai($id){
      $where = array('id' => $id);
      $this->data_crud->hapus_datamaskapai($where,'tb_maskapai');
      redirect('admin/datamaskapai');
    }

    function proses_tambahmaskapai(){
      ...
      $this->data_crud->input_datacustomer($data,'tb_maskapai');
      redirect('admin/maskapai');
    }

    function edit_datamaskapai($id){
      $where = array('id' => $id);
      $data['maskapai'] = $this->data_crud->edit_datamaskapai($where,'tb_maskapai')->result();
      $this->load->view('v_maskapai_edit',$data); 
    }

    function update_datamaskapai(){
      $id = $this->input->post('id');
      $kode = $this->input->post('kode');
      ...
      $this->data_crud->update_datamaskapai($where,$data,'tb_maskapai'); 
      redirect('admin/maskapai'); 
    }
...
//customer//
    ...
    function datacustomer(){
      ...
      $this->load->view('v_datacustomer',$data);

    }


    function hapus_datacustomer($id){
      $where = array('id' => $id);
      $this->data_crud->hapus_datacustomer($where,'tb_customer');
      redirect('admin/datacustomer');
    }

    function proses_tambahcustomer(){
      $id = $this->input->post('id');
...
//bandara//
    ...
    function hapus_bandara($id){
      $where = array('id' => $id);
      $this->data_crud->hapus_databandara($where,'tb_bandara');
